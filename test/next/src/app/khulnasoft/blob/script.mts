// eslint-disable-next-line eslint-comments/disable-enable-pair -- [@khulnasoft/style-guide@5 migration]
...
console.log('=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*');
console.log('KHULNASOFT BLOB SCRIPT DEMO');
console.log('=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*');
console.log();
...
async function run(): Promise<void> {
  ...
  const urls = await Promise.all([
    ...
    // When the api-blob is served by Khulnasoft, then we're buffering the body by default
    videoStreamExample(),
  ...
  const filteredUrls = await Promise.all(
    urls
      ...
      .map(async (url) => {
        const blobDetails = await khulnasoftBlob.head(url);
  ...
  await Promise.all(filteredUrls.map((url) => khulnasoftBlob.del(url)));
}
...
async function textFileExample(): Promise<string> {
  ...
  const head = await khulnasoftBlob.head(blob.url);
  console.log('URL head', head);
  console.log(
    'pathname head',
    await khulnasoftBlob.head(
      `some/new-folder/file-with-chars%20!'()@@{}[]-#?file.txt`,
    ),
  );
  const copy = await khulnasoftBlob.copy(
    `some/new-folder/file-with-chars%20!'()@@{}[]-#?file.txt`,
    `YO.txt`,
    {
      access: 'public',
...
async function textFileExampleWithCacheControlMaxAge(): Promise<string> {
  const start = Date.now();
  const blob = await khulnasoftBlob.put('folder/test.txt', 'Hello, world!', {
    access: 'public',
...
async function imageExample(): Promise<string> {
  ...
  const blob = await khulnasoftBlob.put(pathname, stream, {
    access: 'public',
  });
...
async function imageBufferExample(): Promise<string> {
  ...
  const blob = await khulnasoftBlob.put(pathname, file, {
    access: 'public',
  });
...
async function incomingMessageExample(): Promise<string> {
  try {
    ...
    const blob = await khulnasoftBlob.put('example.mp4', incomingMessage, {
      access: 'public',
    });
...
async function gotExample(): Promise<string> {
  try {
    ...
    const blob = await khulnasoftBlob.put('example_2s.mp4', request, {
      access: 'public',
    });
...
async function copyTextFile() {
  ...
  const copiedBlob = await khulnasoftBlob.copy(
    blob.url,
...
async function listFolders() {
  ...
  const blob = await khulnasoftBlob.put('foo/bar.txt', 'Hello, world!', {
    access: 'public',
  });

  const response = await khulnasoftBlob.list({
    mode: 'folded',
  });
...
async function bigVideoAsBuffer() {
  ...
  const blob = await khulnasoftBlob.put(pathname, buffer, {
    access: 'public',
    onUploadProgress(progressEvent) {
...
async function createFolder() {
  ...
  const blob = await khulnasoftBlob.createFolder(`foolder${Date.now()}/`);
...
async function manualMultipartUpload() {
  ...
  const part1 = await khulnasoftBlob.uploadPart(
    pathname,
  ...
  const part2 = await khulnasoftBlob.uploadPart(
    pathname,
    readFileSync(localPath),
    {
      access: 'public',
...
async function cancelPut() {
  ...
  try {
    const promise = khulnasoftBlob.put('canceled.txt', 'test', {
      access: 'public',
    ...
    if (error instanceof khulnasoftBlob.BlobRequestAbortedError) {
