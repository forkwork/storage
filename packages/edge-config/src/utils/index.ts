...
export const ERRORS = {
  UNAUTHORIZED: '@khulnasoft/edge-config: Unauthorized',
  EDGE_CONFIG_NOT_FOUND: '@khulnasoft/edge-config: Edge Config not found',
};

export class UnexpectedNetworkError extends Error {
  constructor(res: Response) {
    super(
      `@khulnasoft/edge-config: Unexpected error due to response with status code ${res.status}`,
    );
  }
}
...
export function assertIsKey(key: unknown): asserts key is string {
  if (typeof key !== 'string') {
    throw new Error('@khulnasoft/edge-config: Expected key to be a string');
  }
}
...
export function assertIsKeys(keys: unknown): asserts keys is string[] {
  if (!Array.isArray(keys) || keys.some((key) => typeof key !== 'string')) {
    throw new Error(
      '@khulnasoft/edge-config: Expected keys to be an array of string',
    );
  }
}
